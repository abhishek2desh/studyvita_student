<?php
		...
		c.name,c.ch_no,s.name,sd.edutech_student_id,sd.sname,alt.subject,alt.end_time 
		FROM adaptive_learning_test alt,adaptive_test_response atr,chapter c,SUBJECT s,student_details sd
		WHERE alt.student_id='$uid' AND alt.test_id=atr.test_id  AND alt.chapter_id=c.id  AND sd.user_id=alt.student_id and alt.chapterwise_test='0'
		...
		c.name,c.ch_no,s.name,sd.edutech_student_id,sd.sname,alt.subject,alt.end_time 
		FROM adaptive_learning_test alt,adaptive_test_response atr,chapter c,SUBJECT s,student_details sd
		WHERE alt.student_id='$uid' AND alt.test_id=atr.test_id AND SUBJECT='$sb_result' AND alt.chapter_id=c.id  AND sd.user_id=alt.student_id and alt.chapterwise_test='0'
		AND alt.test_type IN('test','practise')  AND s.id=alt.subject  and sd.batch_id=alt.batch_id and alt.batch_id ='$batch_id' ORDER BY  CAST(alt.test_id AS UNSIGNED   INTEGER) DESC");
		}
		...
		while($row=mysql_fetch_array($result))
		{
			...
			while($row2=mysql_fetch_array($result2))
			{
				...
				if($j%2 == 0)
				{
					echo "<tr id='$row[0],$row[6],$row[7],$row[5],$row[8]' style='width:100px;background-color:#5E9DC8;border:solid 1px;height:20px;'>";
	...
	else
	{
				...
				{
					echo "<tr id='$row[0],$row[6],$row[7],$row[5],$row[8]' style='width:100px;background-color:white;border:solid 1px;height:20px;'>";
