<?php
		...
		alt.combined_chapter,alt.subject,sd.edutech_student_id,sd.sname,alt.subject,alt.end_time  
		FROM adaptive_learning_test alt,adaptive_test_response atr,student_details sd
		WHERE alt.student_id='$uid' AND alt.test_id=atr.test_id  AND sd.user_id=alt.student_id and demand_test='1' and alt.test_submit='1' 
		   and sd.batch_id=alt.batch_id  and alt.batch_id ='$batch_id' ORDER BY  CAST(alt.test_id AS UNSIGNED   INTEGER) DESC");
		}
		else
		{
		$result=mysql_query("SELECT DISTINCT  CAST(alt.test_id AS UNSIGNED   INTEGER),DATE_FORMAT(DATE(alt.start_time),'%d-%m-%Y'),TIMEDIFF(alt.end_time,alt.start_time),
		alt.combined_chapter,alt.subject,sd.edutech_student_id,sd.sname,alt.subject,alt.end_time  
		FROM adaptive_learning_test alt,adaptive_test_response atr,student_details sd
		WHERE alt.student_id='$uid' AND alt.test_id=atr.test_id AND SUBJECT='$sb_result'  AND sd.user_id=alt.student_id and demand_test='1' and alt.test_submit='1' 
		   and sd.batch_id=alt.batch_id  and alt.batch_id ='$batch_id' ORDER BY  CAST(alt.test_id AS UNSIGNED   INTEGER) DESC");
		}
...
$chname="";				
		...
		//c.name,c.ch_no,s.name,sd.edutech_student_id,sd.sname 
					...
					echo "<td style='width:100px;border:solid 0px;height:20px;'><center><label style='color:black'><b>".$row[1]."</b></label></center></td>";
						...
						$resultch1=mysql_query("SELECT name FROM subject WHERE id='$row[4]'");
				while($rowch1=mysql_fetch_array($resultch1))
				{
					
					echo "<td style='width:100px;border:solid 0px;height:20px;'><center><label style='color:black'><b>".$rowch1[0]."</b></label></center></td>";
				}
