...
<nav>
	...
	<?php
		...
		{
			$sql_in="SELECT useful_link,hyper_link,id FROM menubar_list WHERE define_user='student' AND active='1' order by Priority";	
		}
			...
			while($row_in=mysql_fetch_array($result_in))
			{
				echo "<li><a href='$row_in[1]'>$row_in[0]</a>";
					...
					{
						$sql_sub="SELECT useful_link,hyper_link,id FROM `submenubar_list` WHERE menu_id='$row_in[2]' AND active='1' order by Priority";
					}
					//echo $sql_sub;
					...
					{
					if($row_sub[0] == "Schedule Test" || $row_sub[0] == "Adaptive Test" || $row_sub[0] == "Chapterwise Test")
					{
						if($subject_id == 20)
						{
						goto nextmenu;
						...
						echo "<li><a href='$row_sub[1]'>$row_sub[0]</a>";
							...
							echo "<ul>";
								echo "<li><a class='chap_id' id='' href='#' >COMBINED CHAPTER</a>";
									echo "<ul>";
										$result_link2=mysql_query("SELECT DISTINCT dms.Srno,dms.MaterialName,dms.path,dms.Globalfile_path,dms.globalfile_name FROM course_batch_material cb,document_manager_subjective dms
									...
									echo "</ul>";
								echo "</li>";
								$result_link3=mysql_query("SELECT DISTINCT c.id,UCASE(c.name) FROM `course_batch_material` cb,`document_manager_subjective` dms,chapter c WHERE cb.doc_id=dms.Srno AND c.id=dms.chapter_id AND cb.batch_id='$batch_id' AND SUBJECT='$subject_id' AND c.id IN(SELECT DISTINCT chap_id FROM course_chapter WHERE course_id='$course_id') ORDER BY c.standard_id desc,c.ch_no or c.id in(SELECT DISTINCT c.id FROM `media_manager` mm,`course_chapter` cc,chapter c,`course_video` cv 
 WHERE cc.sub_id='$subject_id' AND c.id=cc.chap_id AND cc.chap_id=mm.chap_id AND cc.course_id='$course_id' AND cv.course_id=cc.course_id AND cv.video_id=mm.id and mm.pathfilename is not null
 ORDER BY c.standard_id DESC,c.ch_no) ");
/*SELECT DISTINCT c.id,UCASE(c.name) FROM `course_batch_material` cb,`document_manager_subjective` dms,chapter c 
WHERE cb.doc_id=dms.Srno AND c.id=dms.chapter_id AND cb.batch_id='349' AND SUBJECT='16' 
AND c.id IN(SELECT DISTINCT chap_id FROM course_chapter WHERE course_id='169') ORDER BY c.standard_id DESC,c.ch_no 
OR c.id IN(SELECT DISTINCT c.id FROM `media_manager` mm,`course_chapter` cc,chapter c,`course_video` cv 
 WHERE cc.sub_id='16' AND c.id=cc.chap_id AND cc.chap_id=mm.chap_id AND cc.course_id='169' 
 AND cv.course_id=cc.course_id AND cv.video_id=mm.id AND mm.pathfilename IS NOT NULL
 ORDER BY c.standard_id DESC,c.ch_no) OR c.id IN(SELECT DISTINCT cm.chapter_id FROM 
 `chapter_mapping` cm,media_manager me,course_video co,course_chapter cu WHERE cu.course_id='169' AND co.course_id=cu.course_id
 AND co.video_id=me.id AND me.pathfilename IS NOT NULL AND me.chap_id=cm.chapter_id_mapping AND  cm.chapter_id=cu.chap_id)
  OR c.id IN(SELECT DISTINCT cm.chapter_id_mapping FROM 
 `chapter_mapping` cm,media_manager me,course_video co,course_chapter cu WHERE cu.course_id='169' AND co.course_id=cu.course_id
 AND co.video_id=me.id AND me.pathfilename IS NOT NULL AND me.chap_id=cm.chapter_id AND  cm.chapter_id=cu.chap_id)*/
							...
							$result_link3=mysql_query("SELECT DISTINCT c.id,UCASE(c.name) FROM `media_manager` mm,`course_chapter` cc,chapter c,`course_video` cv 
 WHERE cc.sub_id='$subject_id' AND c.id=cc.chap_id AND cc.chap_id=mm.chap_id AND cc.course_id='$course_id' AND cv.course_id=cc.course_id AND cv.video_id=mm.id and mm.pathfilename is not null
 ORDER BY c.standard_id DESC,c.ch_no");	
								$rs_link=mysql_num_rows($result_link3);
								}
