	...
	else if($test_submit_1 == "" && $test_end_time == "" && $test_start_time == "")
	{
		...
		for ($j=0;$j<=$rw;$j++)
		{
			...
			else
			{
				...
				$select_que_str=$select_que_str.$ran.",";
				
				$result1=mysql_query("SELECT TestID,Uniqid,Qno FROM onlineuniqid WHERE TestID='$test_id' AND Qno='$ran'");
				$rw1=mysql_num_rows($result1);
				$row1=mysql_fetch_array($result1);
				$uq_id=$row1[1];
				
				$result_curr=mysql_query("SELECT CorrectAns FROM onlinequestionbank WHERE UniqId=$uq_id");
				...
				VALUES('$test_id','$i','$uq_id','$response','$get_ans','$uid')");
