<?php
		...
		{
			$result1_top=mysql_query("SELECT c.id,ob.UniqId,ob.path,ud.Per FROM concept c,onlinequestionbank ob,uniqidwise_diff_per ud WHERE ud.UniqId=ob.UniqId AND c.topic_id='$topic_row[0]' AND c.id=ob.ConceptId AND ob.Documenttype='O' AND ob.MEDIUM IN('E','EG') AND c.id > '0' AND ob.Blocked_Question='0'");
		...
		//echo "SELECT Avg_percent FROM chapterwise_student_average WHERE chapter_id='$chap_id' AND student_id='$row1[0]'";
