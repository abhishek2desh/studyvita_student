<?php
	...
	$random_generate=$_SESSION['random_generate'];*/
	...
	$chapter_list_que=$_GET['chapter_list_que'];	
	$cpt_count=$_GET['cpt_count'];
	$cno_count=$_GET['cno_count'];
	$_SESSION['chapter_list_que']=$chapter_list_que;
	$_SESSION['cpt_count']=$cpt_count;
	$_SESSION['cno_count']=$cno_count;
...
<script> 
	$(document).ready(function(){
		...
		var chap_id='<?php echo $cpt_count;?>';var subject_id='<?php echo $subject_id;?>';
		var uid='<?php echo $user_id; ?>';
		var req_que='<?php echo $req_que; ?>';
		var chapter_list_que='<?php echo $chapter_list_que; ?>';
		var cpt_count='<?php echo $cpt_count; ?>';
		var cno_count='<?php echo $cno_count; ?>';
		...
		var mySplitResult = data.split("-");
				...
				filename="test_paper_generator/11_view_que1_customise.php";
				data34={total_que_str: tot_qno_id,uniq_id_get:uniq_id_get,marks_for_review:marks_for_review,marks_for_atm:marks_for_atm,req_que:req_que,total_no_que:qno_id,cp_value12:chapter_list_que,sub_id:subject_id,uid:uid,course_id:course_id,cpt_count:cpt_count,cno_count:cno_count};
				
				//alert(data34);
				//alert(tot_qno_id+uniq_id_get+marks_for_review+marks_for_atm+req_que+qno_id+chap_id+subject_id+uid+course_id);
		...
		function getContent_que34(filename, selectboxid)
		{
			$.ajax({
				...
				success: function(data, textStatus, xhr) {
					...
					{
							setTimeout(function(){
								url="test_paper_generator_custom.php";
		...
		//$("#load_hide").hide();
				...
				$("#main_div").hide();
				 ...
				 $.ajax({
					...
					success: function(data, textStatus, xhr) {
						if(data == 'true')
						{
							alert("Test Result has been Sent to your registered email-id. Thank You.");
							setTimeout(function(){
								url="view_my_result_custom.php";
								document.location.href=url;
							}, 1000);
						}
					},
			...
			//alert(s);
			filename = "test_paper_generator/21_insert_adaptive_custom.php?sub="+subject_id+"&user_id="+uid+"&req_que="+req_que+"&date_time="+s+"&date_time2="+date_time2+"&t_t="+t_t+"&select_test_wise="+select_test_wise+"&total_que_str="+total_string_get_val+"&cp_value12="+chap_id+"&batch_id="+batch_id;
			//alert(filename);
			$.ajax({
				url: filename,
				type: 'GET',
