...
<script> 
	$(document).ready(function(){
		...
		$('#load_hide').hide();
		filename = "query_jee_moc/1_test_schedule.php?uid="+uid;
		...
		$('#test_schedule_display').click(function(){
			...
			{
				filename = "query_jee_moc/check_test_id.php?test_id="+get_test_id1+"&uid="+uid;
				//alert(filename);
				$.ajax({
					url: filename,
					type: 'GET',
		...
		$('#load_hide').show();
		$('#main_div').hide();
		filename = "query_jee_moc/count_basic_account_test.php?batch_id="+batch_id+"&uid="+uid;
				//alert(filename);
				$.ajax({
					url: filename,
					type: 'GET',
					...
					success: function(data, textStatus, xhr) {
						...
						{
						//for blueprint test
						filename = "query_jee/insert_blueprintpaper_mock.php?get_test_id="+get_test_id1+"&user_id="+uid+"&course_id="+stu_course_id;
					$.ajax({
			url: filename,
			type: 'GET',
			dataType: 'html',
		...
		var h = screen.width;
			var w = screen.height;
			document.location.href="layout-moc.php?subject="+subject_id+"&test_id="+get_test_id1;
			}
			else
			{
...
<body>
	<?php include 'body_part2_moc.php'; ?>
</body>
</html>