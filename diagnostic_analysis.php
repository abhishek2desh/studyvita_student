...
<head>		
		...
		var page_source="diagnostic_analysis.php";
...
filename = "query/update_submenu_log.php?uid="+user_id+"&docid="+currentdocid+"&start_time="+doc_start_time+"&end_time="+doc_end_time+"&page_source="+page_source+"&submenu_with_menu="+submenu_with_menu+"&sub_menu_name="+sub_menu_name;
		...
		if($rw==0)
		{
									$result = mysql_query("SELECT s.unit_id,s.Cumulative_per,u.unit_name,s.id FROM studenttestwise_unitper s,unit u WHERE s.user_id='$s5' AND s.subject_id='$sub_id' AND s.unit_id=u.id AND 
									s.id IN( SELECT MAX(id) FROM studenttestwise_unitper WHERE user_id='$s5' AND subject_id='$sub_id' GROUP BY unit_id)");
			...
			foreach($lst as $item) 
			{
				...
				$result = mysql_query("SELECT distinct s.unit_id,u.id,u.unit_name FROM studenttestwise_unitper s,unit u  WHERE s.user_id in($final_colon)  AND s.subject_id='$sub_id' and u.id=s.unit_id order by s.unit_id");
			/*$result = mysql_query("SELECT s.unit_id,s.Cumulative_per,u.unit_name,s.id FROM studenttestwise_unitper s,unit u WHERE s.user_id='$s5' AND s.subject_id='$sub_id' AND s.unit_id=u.id AND 
									s.id IN( SELECT MAX(id) FROM studenttestwise_unitper WHERE user_id='$s5' AND subject_id='$sub_id' GROUP BY unit_id)");*/
