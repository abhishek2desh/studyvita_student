...
<script> 
	$(document).ready(function(){
		...
		var req_que='<?php echo $req_que; ?>';
			var random_generate='<?php echo $random_generate; ?>';
		var sel_time='<?php echo $sel_time; ?>';
		var batch_id='<?php echo $batch_id;?>';
		//alert(batch_id);
		var course_id='<?php echo $course_id;?>';
		var date_time2,date_time,datetime,t_t,select_test_wise="unittest",select_type_wise=1;
		var all_atm="";
		...
		var mySplitResult = data.split("-");
				...
				filename="test_paper_generator/11_view_que_unitwise.php";
				data34={total_que_str: tot_qno_id,uniq_id_get:uniq_id_get,marks_for_review:marks_for_review,marks_for_atm:marks_for_atm,req_que:req_que,total_no_que:qno_id,cp_value12:chap_id,sub_id:subject_id,uid:uid,course_id:course_id};
		...
		function getContent_que34(filename, selectboxid)
		{
			$.ajax({
				...
				success: function(data, textStatus, xhr) {
					...
					{
							setTimeout(function(){
								url="test-paper-generator-unitwise.php";
		...
		//$("#load_hide").hide();
				 ...
				 $.ajax({
					...
					success: function(data, textStatus, xhr) {
						if(data == 'true')
						{
							...
							setTimeout(function(){
								url="view_my_result_unittest.php";
								document.location.href=url;
							}, 1000);
						}
					},
			...
			filename = "test_paper_generator/21_insert_adaptive_unitwise.php?sub="+subject_id+"&user_id="+uid+"&req_que="+req_que+"&date_time="+s+"&date_time2="+date_time2+"&t_t="+t_t+"&select_test_wise="+select_test_wise+"&total_que_str="+total_string_get_val+"&cp_value12="+chap_id+"&batch_id="+batch_id;
			//alert(filename);
			$.ajax({
				url: filename,
				type: 'GET',
