...
(ob.Blocked_Question='0' OR  ob.Blocked_Question IS NULL)  AND ob.documenttype='O' AND ob.difficulty_per BETWEEN '$avg_val' AND '$avg' and m.conceptid NOT IN(SELECT id FROM concept WHERE topic_id=m.Gseb_TopicId) AND (ob.FacultyId = '$tutor_id' or ob.FacultyId in(select id from user where created_by='$tutor_id')) ");
$ud_rw = mysql_num_rows($ud_count);
						}
						elseif($br_id=='2')
						{
...
$ud_rw = mysql_num_rows($ud_count);
			...
			WHERE m.Cbse_TopicId IN(SELECT id FROM topic WHERE chap_id='$cp_value12') AND m.conceptid=ob.ConceptId  AND c.id=m.conceptid 
				AND ob.conceptid > 0 AND Repeat_uniqid=FALSE AND pdffile_exist=TRUE AND t.id=m.Gseb_TopicId  AND ob.MEDIUM IN('E','EG') AND 
				(ob.Blocked_Question='0' OR  ob.Blocked_Question IS NULL)  AND ob.documenttype='O' AND ob.difficulty_per BETWEEN '$avg' AND '$avg_val'  and m.conceptid NOT IN(SELECT id FROM concept WHERE topic_id=m.Gseb_TopicId) AND (ob.FacultyId = '$tutor_id' or ob.FacultyId in(select id from user where created_by='$tutor_id')) ");
				$ud_rw = mysql_num_rows($ud_count);
					}
					elseif($br_id=='2')
					{
