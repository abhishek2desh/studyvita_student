...
(ob.Blocked_Question='0' OR  ob.Blocked_Question IS NULL)  AND ob.documenttype='O' AND ob.difficulty_per BETWEEN '$avg_val' AND '$avg'  AND (ob.FacultyId = '$tutor_id' or ob.FacultyId in(select id from user where created_by='$tutor_id')) and m.conceptid NOT IN(SELECT id FROM concept WHERE topic_id=m.Gseb_TopicId)");
...
WHERE m.Gseb_TopicId IN(SELECT id FROM topic WHERE chap_id='$cp_value12') AND m.conceptid=ob.ConceptId  AND c.id=m.conceptid 
AND ob.conceptid > 0 AND Repeat_uniqid=FALSE AND pdffile_exist=TRUE AND t.id=m.Cbse_TopicId  AND ob.MEDIUM IN('E','EG') AND 
(ob.Blocked_Question='0' OR  ob.Blocked_Question IS NULL)  AND ob.documenttype='O' AND ob.difficulty_per BETWEEN '$avg_val' AND '$avg' and m.conceptid NOT IN(SELECT id FROM concept WHERE topic_id=m.Gseb_TopicId) ");
echo "SELECT c.id,ob.UniqId,ob.path FROM concept c,onlinequestionbank ob,Concept_mapping m,topic t
WHERE m.Gseb_TopicId IN(SELECT id FROM topic WHERE chap_id='$cp_value12') AND m.conceptid=ob.ConceptId  AND c.id=m.conceptid 
AND ob.conceptid > 0 AND Repeat_uniqid=FALSE AND pdffile_exist=TRUE AND t.id=m.Cbse_TopicId  AND ob.MEDIUM IN('E','EG') AND 
(ob.Blocked_Question='0' OR  ob.Blocked_Question IS NULL)  AND ob.documenttype='O' AND ob.difficulty_per BETWEEN '$avg_val' AND '$avg' and m.conceptid NOT IN(SELECT id FROM concept WHERE topic_id=m.Gseb_TopicId) ";
$ud_rw = mysql_num_rows($ud_count);
			...
			WHERE m.Gseb_TopicId IN(SELECT id FROM topic WHERE chap_id='$cp_value12') AND m.conceptid=ob.ConceptId  AND c.id=m.conceptid 
				AND ob.conceptid > 0 AND Repeat_uniqid=FALSE AND pdffile_exist=TRUE AND t.id=m.Cbse_TopicId  AND ob.MEDIUM IN('E','EG') AND 
				(ob.Blocked_Question='0' OR  ob.Blocked_Question IS NULL)  AND ob.documenttype='O' AND ob.difficulty_per BETWEEN '$avg' AND '$avg_val' AND (ob.FacultyId = '$tutor_id' or ob.FacultyId in(select id from user where created_by='$tutor_id')) and m.conceptid NOT IN(SELECT id FROM concept WHERE topic_id=m.Gseb_TopicId) ");
			...
			WHERE m.Gseb_TopicId IN(SELECT id FROM topic WHERE chap_id='$cp_value12') AND m.conceptid=ob.ConceptId  AND c.id=m.conceptid 
				AND ob.conceptid > 0 AND Repeat_uniqid=FALSE AND pdffile_exist=TRUE AND t.id=m.Cbse_TopicId  AND ob.MEDIUM IN('E','EG') AND 
				(ob.Blocked_Question='0' OR  ob.Blocked_Question IS NULL)  AND ob.documenttype='O' AND ob.difficulty_per BETWEEN '$avg' AND '$avg_val'  and m.conceptid NOT IN(SELECT id FROM concept WHERE topic_id=m.Gseb_TopicId) ");
				$ud_rw = mysql_num_rows($ud_count);
			echo "SELECT c.id,ob.UniqId,ob.path FROM concept c,onlinequestionbank ob,Concept_mapping m,topic t
			WHERE m.Gseb_TopicId IN(SELECT id FROM topic WHERE chap_id='$cp_value12') AND m.conceptid=ob.ConceptId  AND c.id=m.conceptid 
				AND ob.conceptid > 0 AND Repeat_uniqid=FALSE AND pdffile_exist=TRUE AND t.id=m.Cbse_TopicId  AND ob.MEDIUM IN('E','EG') AND 
		...
		if($br_id=='1' || $br_id=='2')
		{
			...
			if($avg > 100)
			{
				if ($use_ges_qbank==0)
				{
					...
					if($br_id=='1')
					{
					$ud_count1=mysql_query("SELECT c.id,ob.UniqId,ob.path FROM concept c,onlinequestionbank ob,Concept_mapping m,topic t
...
$flg=1;
$ud_rw = mysql_num_rows($ud_count1);
					}
					elseif($br_id=='2')
					{
						$ud_count1=mysql_query("SELECT c.id,ob.UniqId,ob.path FROM concept c,onlinequestionbank ob,Concept_mapping m,topic t
