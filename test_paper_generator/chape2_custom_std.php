...
$result1=mysql_query("SELECT board_id,standard_id FROM batch WHERE id='$batch_id'");
		...
		{
			$result=mysql_query("SELECT  c.name,c.id,c.ch_no FROM chapter c,course_chapter cc WHERE cc.course_id='$course_id' AND c.id=cc.chap_id AND active=1 and cc.sub_id IN (SELECT sub_id FROM subject_alias WHERE rel_sub_id='$subject_id') and c.standard_id='$std_id'");
			}
