...
$result1=mysql_query("SELECT board_id,standard_id FROM batch WHERE id='$batch_id'");
		...
		{
			$result=mysql_query("SELECT c.ch_no,UCASE(c.name),c.id FROM chapter c,course_chapter cc,course_batch_active_chapter cb WHERE cc.course_id='$course_id' AND c.id=cc.chap_id AND c.active=1 and cc.sub_id IN (SELECT sub_id FROM subject_alias WHERE rel_sub_id='$subject_id') and c.id=cb.chapter_id and cb.active='1' and cb.course_id=cc.course_id and cb.batch_id='$batch_id' and c.standard_id='$std_id'");
			
		}
		else
		{
		$result=mysql_query("SELECT c.ch_no,UCASE(c.name),c.id FROM chapter c,course_chapter cc WHERE cc.course_id='$course_id' AND c.id=cc.chap_id AND active=1 and cc.sub_id IN (SELECT sub_id FROM subject_alias WHERE rel_sub_id='$subject_id') and c.standard_id='$std_id'");
		}
...
(ob.Blocked_Question='0' OR  ob.Blocked_Question IS NULL)  AND ob.documenttype='O' and m.conceptid NOT IN(SELECT id FROM concept WHERE topic_id=m.Gseb_TopicId)");
			...
			/* $total_que=0;
			 $result_que=mysql_query("SELECT (SUM(totalque)+SUM(mapped_que)) AS total FROM `question_status` WHERE chap_id='$row[2]'");
				...
				echo "<td style='border-bottom:solid 0px;width:100%;' id='ck_first_value' value=$row[0]-$row[2]><font face='verdana' size='3' color='black' >$row[1]</font></td>";
				...
				echo "<td><font face='verdana' size='4' color='white' >".$avg."</font></td>";*/
				echo "</tr>";
			}
		}
		echo "</table>";
