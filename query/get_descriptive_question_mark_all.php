<?php
 ...
 c.topic_id=ccm.module_name AND ccm.course_id='$course_id' AND ccm.sub_id='$subject_id'   and ob.mark='$text_mark' AND c.id=ob.ConceptId AND ob.Documenttype='S' AND ob.MEDIUM IN('E','EG') AND c.id > '0' and Repeat_uniqid=false and pdffile_exist=True AND (ob.Blocked_Question='0' or uniqid not IN (select distinct uniqid from block_question_private where user_id='$tutor_id'))  AND (ob.FacultyId = '$tutor_id' or ob.FacultyId in(select id from user where created_by='$tutor_id')) ");
				}
				else
				{
				///echo in else1";
				$ud_count=mysql_query("SELECT distinct ob.UniqId,ob.subject,ob.mark FROM concept c,onlinequestionbank ob,course_chapter_module ccm WHERE
 c.topic_id=ccm.module_name AND ccm.course_id='$course_id' AND ccm.sub_id='$subject_id'   AND c.id=ob.ConceptId AND ob.Documenttype='S' and ob.mark='$text_mark' AND ob.MEDIUM IN('E','EG') AND c.id > '0' and Repeat_uniqid=false and pdffile_exist=True AND ob.Blocked_Question='0'  ");
				}
