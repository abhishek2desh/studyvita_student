<?php
		...
		$result1=mysql_query("SELECT chapter_id,combined_chapter,batch_id FROM `adaptive_learning_test` WHERE test_id='$test_id' ");
			...
			if($chap_mappingid=="")
			{
			$result_link2=mysql_query("SELECT DISTINCT dms.Srno,dms.MaterialName,dms.path,dms.Chapter,dms.description FROM course_batch_material cb,document_manager_subjective dms,chapter c WHERE cb.doc_id=dms.Srno AND c.id=dms.chapter_id AND cb.batch_id='$batch_id' AND (dms.chapter_id='$chap_id' or dms.chapter_id='$ch_new' or dms.chapter_id='$chap_mappingid' or dms.chapter_id='$chapter_mappingid_new') AND dms.documenttype='PPT'");
			}
			else
			{
			$result_link2=mysql_query("SELECT DISTINCT dms.Srno,dms.MaterialName,dms.path,dms.Chapter,dms.description FROM course_batch_material cb,document_manager_subjective dms,chapter c WHERE cb.doc_id=dms.Srno AND c.id=dms.chapter_id AND cb.batch_id='$batch_id' AND (dms.chapter_id='$chap_id' or dms.chapter_id='$ch_new' or dms.chapter_id='$chap_mappingid' or dms.chapter_id='$chapter_mappingid_new') AND dms.documenttype='PPT'");
		...
		{
			//echo "<option value=''>Select ppt</option>";
			echo "<table style='width:100%'>";
					echo "<tr><td>Select ppt</td></tr>";
		...
		if($j%2 == 0)
					...
					echo "<td style='width:100%;border-bottom: 1px solid black;'><input type='radio' name='pt_radio' id='$row[0]|$row[1]|$row[2]' class='' value='' value='' />$row[0]</td>";
					echo "</tr>";
						$j++;
