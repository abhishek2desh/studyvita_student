...
/*org table-headre color-#4d90c9*
	...
	if($material_type=="ppt" || $material_type=="notes" ||  $material_type=="topperanswersheet" || $material_type=="Weblink" )
	{
		...
		$result_link2=mysql_query("SELECT DISTINCT dms.Srno,dms.MaterialName,dms.path,dms.Chapter,dms.description,cb.Allot_studentwise,cb.studentlist,dms.chapter_id FROM course_batch_material cb,document_manager_subjective dms,chapter c WHERE cb.doc_id=dms.Srno AND c.id=dms.chapter_id AND cb.batch_id='$batch_id' AND SUBJECT='$subject_id' AND (dms.chapter_id='$chapter_id' or dms.chapter_id='$ch_new' or dms.chapter_id='$chap_mappingid' or dms.chapter_id='$chapter_mappingid_new') $querystring order by dms.srno desc");
			...
			<td style='width:3%;border-bottom:solid 1px;color:black;background-color: #00bca4;'></td>
			<td style='width:20%;border-bottom:solid 1px;border-right:solid 1px;color:black;background-color: #00bca4;'><b><font  color='white' >Ref.ID</font></b></td>
			<td style='width:57%;border-bottom:solid 1px;color:black;background-color: #00bca4;'><b><font  color='white' >Chapter</font></b></td>
			<td style='width:20%;border-bottom:solid 1px;color:black;background-color: #00bca4;'><b><font  color='white' >Number of access</font></b></td>";
			echo"</tr>";
			$total=1;
						}
						if($row[5]=='1')
						...
						if($j%2 == 0)
				{
					echo "<tr style='background-color:#ccf1ec;height:25px;'>";
					}
					else
					{
					echo "<tr style='background-color:#e5f8f5;height:25px;'>";
					}
				$j++;
...
$result_acc=mysql_query("SELECT COUNT(id) FROM `menu_log_detail` WHERE user_id='$uid' AND document_id='$row[0]'");
	...
	$total=0;
		...
		echo "<tr style='height:40px;'>
							<td style='width:3%;border-bottom:solid 1px;color:black;background-color: #00bca4;'></td>
							<td style='width:20%;border-bottom:solid 1px;border-right:solid 1px;color:black;background-color: #00bca4;'><b><font  color='black' >Ref.ID</font></b></td>";
							echo "<td style='width:25%;border-bottom:solid 1px;color:black;background-color: #00bca4;'><b><font  color='black' >Magazine Name</font></b></td>";
							echo "<td style='width:27%;border-bottom:solid 1px;color:black;background-color: #00bca4;'><b><font  color='black' >Chapter</font></b></td>";
							
							echo "<td style='width:25%;border-bottom:solid 1px;color:black;background-color: #00bca4;'><b><font  color='black' >Month Year</font></b></td>";
			echo"</tr>";
			$total=1;
						}
						if($row[5]=='1')
	...
	else if($material_type=="previouscompetitivepaper")
	{
		...
		if($chap_mappingid=="")
			...
			{
					 if (strpos($row_link2[9],",") !== false)
					 {
						 $chapter="";
						 $lst=explode(",",$row_link2[9]);
						foreach($lst as $item)
						{
						if($item=="")
						{
					 ...
					 {
						 $result_chap=mysql_query("SELECT name FROM `chapter` where id='$row_link2[9]'");
						while($row_chap=mysql_fetch_array($result_chap))
						{
						$chapter=$row_chap[0];
						}
				...
				$j++;
							...
							$result_acc=mysql_query("SELECT COUNT(id) FROM `menu_log_detail` WHERE user_id='$uid' AND document_id='$row_link2[0]'");
							while($row_acc=mysql_fetch_array($result_acc))
							{
							$noaccess=$row_acc[0];
							}
echo "<td style='border:solid 0px;border-bottom:solid 1px;width:20%;'><font  color='black' >".$noaccess."</font></td>";							
	...
	$result_link2=mysql_query("SELECT DISTINCT mm.SubTopic,cv.course_id,cv.video_id,mm.Pathfilename,mm.Subject,mm.board,mm.standard,mm.chap_id 
		FROM course_video cv,media_manager mm WHERE cv.course_id='$course_id' AND mm.id=cv.video_id AND mm.chap_id='$chapter_id' AND mm.pathfilename is not null");
			...
			<td style='width:3%;border-bottom:solid 1px;color:black;background-color: #00bca4;'></td>
			<td style='width:20%;border-bottom:solid 1px;border-right:solid 1px;color:black;background-color: #00bca4;'><b><font  color='white' >Ref.ID</font></b></td>
			<td style='width:57%;border-bottom:solid 1px;color:black;background-color: #00bca4;'><b><font  color='white' >File Name</font></b></td>
			<td style='width:20%;border-bottom:solid 1px;color:black;background-color: #00bca4;'><b><font  color='white' >Number of access</font></b></td>";
			echo"</tr>";
						$total=1;
						}
