	...
	echo "[";
				...
				foreach($lst1 as $item1) 
			{
				...
				else
				{
					...
					$rs1 = mysql_query("SELECT (100-s.Cumulative_per),s.id FROM studenttestwise_unitper s,unit u WHERE s.user_id='$item1' AND s.subject_id='15' AND s.unit_id=u.id AND s.unit_id='$row2[0]' and s.id IN( SELECT MAX(id) FROM studenttestwise_unitper WHERE user_id='$item1' AND subject_id='15' and unit_id='$row2[0]' )");
