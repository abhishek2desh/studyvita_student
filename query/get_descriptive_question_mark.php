<?php
	...
	//for checking ges question rights
		...
		if ($use_ges_qbank==0)
				...
				$ud_count=mysql_query("SELECT distinct ob.UniqId,ob.subject,ob.mark FROM concept c,onlinequestionbank ob WHERE c.topic_id IN(SELECT id FROM topic WHERE chap_id='$chapter_id') and ob.mark='$text_mark' AND c.id=ob.ConceptId AND ob.Documenttype='S' AND ob.MEDIUM IN('E','EG') AND c.id > '0' and Repeat_uniqid=false and pdffile_exist=True AND (ob.Blocked_Question='0' or uniqid not IN (select distinct uniqid from block_question_private where user_id='$tutor_id'))  AND (ob.FacultyId = '$tutor_id' or ob.FacultyId in(select id from user where created_by='$tutor_id')) ");
				}
				else
				{
				///echo in else1";
				$ud_count=mysql_query("SELECT distinct ob.UniqId,ob.subject,ob.mark FROM concept c,onlinequestionbank ob WHERE c.topic_id IN(SELECT id FROM topic WHERE chap_id='$chapter_id')AND c.id=ob.ConceptId AND ob.Documenttype='S' and ob.mark='$text_mark' AND ob.MEDIUM IN('E','EG') AND c.id > '0' and Repeat_uniqid=false and pdffile_exist=True AND ob.Blocked_Question='0'  ");
				}
		...
		if ($use_ges_qbank==0)
				{
					if($br_id=='1')
					{
					$ud_count1=mysql_query("SELECT  distinct ob.UniqId,ob.subject,ob.mark FROM concept c,onlinequestionbank ob,Concept_mapping m,topic t
...
(ob.Blocked_Question='0' OR  ob.Blocked_Question IS NULL)  AND ob.documenttype='S' and ob.mark='$text_mark' AND (ob.FacultyId = '$tutor_id' or ob.FacultyId in(select id from user where created_by='$tutor_id'))");
					}
					elseif($br_id=='2')
					{
						$ud_count1=mysql_query("SELECT distinct ob.UniqId,ob.subject,ob.mark FROM concept c,onlinequestionbank ob,Concept_mapping m,topic t
...
(ob.Blocked_Question='0' OR  ob.Blocked_Question IS NULL)  AND ob.documenttype='S' and ob.mark='$text_mark'   AND (ob.FacultyId = '$tutor_id' or ob.FacultyId in(select id from user where created_by='$tutor_id'))");
			...
			WHERE m.Cbse_TopicId IN(SELECT id FROM topic WHERE chap_id='$chapter_id') AND m.conceptid=ob.ConceptId  AND c.id=m.conceptid 
AND ob.conceptid > 0 AND Repeat_uniqid=FALSE AND pdffile_exist=TRUE AND t.id=m.Gseb_TopicId  AND ob.MEDIUM IN('E','EG') AND 
(ob.Blocked_Question='0' OR  ob.Blocked_Question IS NULL)  AND ob.documenttype='S' and ob.mark='$text_mark'  ");
					}
					elseif($br_id=='2')
					{
					$ud_count1=mysql_query("SELECT distinct ob.UniqId,ob.subject,ob.mark FROM concept c,onlinequestionbank ob,Concept_mapping m,topic t
