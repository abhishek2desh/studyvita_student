...
session_start();
	...
	$result=mysql_query("SELECT DISTINCT stm.top_id FROM document_manager_subjective dms,material m,sub_top_mapping stm WHERE dms.MaterialName='$m_n' AND dms.Srno=m.DocumentManager_RefID AND m.id=stm.mat_id");
	...
	if($type_type == "OA")
	{
		if($submission_date == "" )
		{
			...
			if($ans_key=="1")
			{
				$sql_1 = "SELECT DISTINCT material_id FROM `omr_student_response` WHERE student_id='$stud_id' AND material_id='$mat_id'";
