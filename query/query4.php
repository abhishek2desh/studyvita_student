	...
	if($bt_call == "cancel")
	{
		if(isset($_GET['dt']))
		{
			...
			$result=mysql_query("SELECT * FROM material m, group_subject_mapping gm,per_material_mapping pm
				...
				&&
				pm.student_id=$s5");
//			echo "<option >Select Chemistry Material</option>";
...
//				echo "<option >Select Chemistry Material</option>";
	...
	else
	{
		...
		if($sb == 17)
		{
			if(isset($_GET['dt']))
			{
					$result=mysql_query("SELECT * FROM material m, group_subject_mapping gm,per_material_mapping 	pm,document_manager_subjective dc WHERE m.material_type_id=".$_GET['dt']." AND m.subject_id = gm.sub_id AND m.material_name=dc.MaterialName AND $sql AND '$cp1' AND gm.group_id='$s3' AND m.id=pm.material_id AND pm.student_id='$s5' AND DATE(Testdate) BETWEEN '$date1' AND '$date2' ");
				while($row=mysql_fetch_array($result))
				{
					include 'material_color.php';
				}
