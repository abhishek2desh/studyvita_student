<?php
		...
		{
		$rs = mysql_query("SELECT  DISTINCT u.name,fd.`experience`,fd.`Qualification`,u.id FROM `faculty_detail` fd,USER u,`faculty_subject_board_std_mapping` fm WHERE u.id=fd.user_id AND fm.user_id=fd.user_id AND fm.board_id='$board_id' AND fm.standard_id='$std_id' AND fm.subject_id='$sub_id' and u.id='$test_id_fact'");
		}
