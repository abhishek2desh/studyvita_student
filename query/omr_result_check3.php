<?php
	...
	$j=0;
	$k=0;
	$sql_1 = "SELECT DISTINCT om.material_id,om.student_id,om.que_no,ms.Answer,ms.Question_No FROM omr_student_response om,`material_correct_answers` ms
WHERE om.material_id=ms.material_id AND om.que_no=ms.Question_No AND om.student_id='$stud_id' AND om.material_id='$mid' ";
	...
	{
	$result_res=mysql_query("SELECT max(id) FROM `omr_student_response`  WHERE student_id='$stud_id' AND material_id='$mid' and que_no='$row_1[2]'");
	while($row_res = mysql_fetch_row($result_res))
	{
	$result_res1=mysql_query("SELECT response FROM `omr_student_response`  WHERE id='$row_res[0]'");
	while($row_res1 = mysql_fetch_row($result_res1))
	{
	$res=$row_res1[0];
	}
	...
	echo "<table border='1' style='width:100%;background:white;'>";
