<?php
		...
		$j=0;
		$result1=mysql_query("SELECT chapter_id FROM `adaptive_learning_test` WHERE test_id='$test_id' ");
			...
			$test_id_fact="";
		$competition_test=0;
		$result13=mysql_query("SELECT DISTINCT t.faculty_id,t.competition_test FROM test_announce t,que_paper q WHERE t.que_paper_id=q.id AND q.name='$test_id' and competition_test='1' ");
		
			while($row1_ch3=mysql_fetch_array($result13))
			{
				$test_id_fact=$row1_ch3[0];
				$competition_test=$row1_ch3[1];
			}
...
u.id=v.user_id  and v.batch_id='$batch_id' and v.class_date='$today' and  v.sub_id='$sub_id' and v.Chapter_id='$chap_id' ORDER BY v.class_date,v.from_time");
	 ...
	 IF(v.to_date < '$formatDate', 'expire','start ')) AS diff,fees,seat_available,invited_id
FROM `virtual_class_faculty` v,course c,batch b,USER u WHERE c.id=v.course_id AND b.id=v.batch_id AND
u.id=v.user_id  and v.batch_id='$batch_id' and v.class_date='$today' and  v.sub_id='$sub_id' and v.Chapter_id='$chap_id' and v.user_id='$test_id_fact' ORDER BY v.class_date,v.from_time");
}
					...
					else
					{
						if($row[12]==0)
						{
							if($row[11] == 'coming_soon')
									...
									{
									echo "<td style='border:solid 1px;'><input type='button' class='search1_s' id='search1_s' value='Start' style='width:100%;'/></td>";
									}
						}
						else
						{
