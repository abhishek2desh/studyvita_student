<?php
	include '../config.php';
	$subject_id=$_GET['sub_id']='14';
	$course_id=$_GET['course_id']='675';
	$material_type=$_GET['material_type']='desq';
	$uid=$_GET['uid']='20975';
	$examtype=$_GET['examtype']='';
	$batch_id=$_GET['batch_id']='900';
	...
	//for checking ges question rights
		...
		//end checking
		echo "<table style='border-top:solid 0px;border-left:solid 0px;border-bottom:solid 0px;color:black;width:100%;overflow-y: scroll;height:475px;overflow-y:auto;' cellspacing='3'>";
			echo "<tr style='height:40px;'>
			<td style='width:3%;border-bottom:solid 1px;color:black;background-color: #4d90c9;height:20px;'></td>
			<td style='width:35%;border-bottom:solid 1px;border-right:solid 1px;color:black;background-color: #4d90c9;height:20px;'><b><font  color='black' >Question ID</font></b></td>
			<td style='width:27%;border-bottom:solid 1px;border-right:solid 1px;color:black;background-color: #4d90c9;height:20px;'><b><font  color='black' >Q.No</font></b></td>
			<td style='width:35%;border-bottom:solid 1px;color:black;background-color: #4d90c9;height:20px;'><b><font  color='black' >Mark</font></b></td>";
			echo"</tr>";
		if ($use_ges_qbank==0)
				{
				//echo "in if";
				$ud_count=mysql_query("SELECT distinct ob.UniqId,ob.mark FROM concept c,onlinequestionbank ob,course_chapter_module ccm WHERE
 c.topic_id=ccm.module_name AND ccm.course_id='$course_id' AND ccm.sub_id='$subject_id'  AND c.id=ob.ConceptId AND ob.Documenttype='S' AND ob.MEDIUM IN('E','EG') AND c.id > '0' and Repeat_uniqid=false and pdffile_exist=True AND (ob.Blocked_Question='0' or uniqid not IN (select distinct uniqid from block_question_private where user_id='$tutor_id'))  AND (ob.FacultyId = '$tutor_id' or ob.FacultyId in(select id from user where created_by='$tutor_id')) ");
				...
				{
				//echo "in else1";
				$ud_count=mysql_query("SELECT distinct ob.UniqId,ob.mark FROM concept c,onlinequestionbank obcourse_chapter_module ccm WHERE
 c.topic_id=ccm.module_name AND ccm.course_id='$course_id' 
AND ccm.sub_id='$subject_id'  AND c.id=ob.ConceptId AND ob.Documenttype='S' AND ob.MEDIUM IN('E','EG') AND c.id > '0' and Repeat_uniqid=false and pdffile_exist=True AND ob.Blocked_Question='0'  ");
				}
			...
			$flg=1;
				...
				echo "<td style='color:black;border-right:solid 0px;border-bottom:solid 1px;width:3%;'><center><input type='radio' name='name_ck' id='$row[0]' class='ck' value='$row[0]' /></center></td>";
							...
							echo "<td style='border:solid 0px;border-bottom:solid 1px;width:35%;'><font  color='black' >".$row[1]."</font></td>";
		...
		if ($use_ges_qbank==0)
				{
					if($br_id=='1')
					{
					$ud_count1=mysql_query("SELECT  distinct ob.UniqId,ob.mark FROM concept c,onlinequestionbank ob,Concept_mapping m,topic t,course_chapter_module ccm WHERE m.Cbse_TopicId=ccm.module_name AND ccm.course_id='$course_id' AND ccm.sub_id='$subject_id'  AND m.conceptid=ob.ConceptId  AND c.id=m.conceptid 
...
(ob.Blocked_Question='0' OR  ob.Blocked_Question IS NULL)  AND ob.documenttype='S'  AND (ob.FacultyId = '$tutor_id' or ob.FacultyId in(select id from user where created_by='$tutor_id'))");
					...
					elseif($br_id=='2')
					{
						$ud_count1=mysql_query("SELECT distinct ob.UniqId,ob.mark FROM concept c,onlinequestionbank ob,Concept_mapping m,topic t,course_chapter_module ccm
