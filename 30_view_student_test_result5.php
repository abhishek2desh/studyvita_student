<?php
		...
		if($sb_result == "")
		{
		$result=mysql_query("SELECT DISTINCT  CAST(alt.test_id AS UNSIGNED   INTEGER),DATE_FORMAT(DATE(alt.start_time),'%d-%m-%Y'),TIMEDIFF(alt.end_time,alt.start_time),s.name,sd.edutech_student_id,sd.sname,alt.subject,alt.end_time FROM adaptive_learning_test alt,adaptive_test_response atr,SUBJECT s,student_details sd WHERE alt.student_id='$uid' AND alt.test_id=atr.test_id AND sd.user_id=alt.student_id AND alt.test_type IN('adaptive_test') AND Test_Submit='1' AND s.id=alt.subject and sd.batch_id=alt.batch_id and alt.batch_id ='$batch_id' ORDER BY  CAST(alt.test_id AS UNSIGNED   INTEGER) DESC");
		}
		else
		{
$result=mysql_query("SELECT DISTINCT  CAST(alt.test_id AS UNSIGNED   INTEGER),DATE_FORMAT(DATE(alt.start_time),'%d-%m-%Y'),TIMEDIFF(alt.end_time,alt.start_time),s.name,sd.edutech_student_id,sd.sname,alt.subject,alt.end_time FROM adaptive_learning_test alt,adaptive_test_response atr,SUBJECT s,student_details sd WHERE alt.student_id='$uid' AND alt.test_id=atr.test_id AND SUBJECT='$sb_result' AND sd.user_id=alt.student_id AND alt.test_type IN('adaptive_test') AND Test_Submit='1' AND s.id=alt.subject  and sd.batch_id=alt.batch_id and alt.batch_id ='$batch_id' ORDER BY  CAST(alt.test_id AS UNSIGNED   INTEGER) DESC");
		}
