<?php
	...
	DATE_FORMAT(from_date,'%d-%m-%Y , %H-%i-%s'),DATE_FORMAT(to_date,'%d-%m-%Y , %H-%i-%s'),duration,IF('$formatDate' < from_date , 'coming_soon', 
	 IF(to_date < '$formatDate', 'expire','star_test ')) AS diff,given_test,sub_id,marks,test_Date,sub.name,CONCAT(ta.chap_id,'',ta.description),dtr.documentid
	FROM test_announce ta,onlineuniqid oq,que_paper q,SUBJECT sub,documentid_testid_refer dtr WHERE  ta.que_paper_id=q.id AND ta.sub_id=sub.id and dtr.testid=q.name
	AND q.name=oq.TestID AND user_id='$uid' AND ta.OnlineTest = '1' and ta.sub_id<>'20' and ta.exam_type='30'  ORDER BY ta.test_date DESC");
	...
	while($row=mysql_fetch_array($result))
	{
		echo "<tr id='$row[0]'>";
			...
			{
				echo "<td id='expire_id' value='expire' style='width:15%;color:black;border-bottom:2px solid #005000;'><div style='color:red;font-size:14px;'>Expired
				</td>";
