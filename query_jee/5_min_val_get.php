<?php	
	...
	/*if($minq==0)
	{
		...
		$br_id=$sql8_row[2];
		$use_ges_qbank=0;
				$sql81=mysql_query("SELECT user_id FROM blueprint_detail WHERE id='$bluprint_id'");
	...
	$sb=$sub_id;
			...
			$select_que_str="";
			$select_ran="";
			$result5=mysql_query("SELECT DISTINCT b.id,c.standard_id,c.board_id,b.chapter_id,b.total_que FROM `blueprint_chapter` b,chapter c WHERE c.id=b.chapter_id AND b.blueprint_id='$bluprint_id'");
		...
		//end insert
		$result4=mysql_query("SELECT SUM(total_que) FROM `blueprint_chapter` WHERE `blueprint_id`='$bluprint_id'");
		$row4=mysql_fetch_array($result4);
		echo "1-".$row4[0]."-".$duration;
		}
